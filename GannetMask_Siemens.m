...
fidoutmask = fullfile([path '/masks'],[name '_mask.nii']);
fidoutmask_image = fullfile([path '/masks'],[name '_mask.jpg']);
...
MRS_struct.p.voxsize = [rda.VOIPhaseFOV rda.VOIReadoutFOV rda.VOIThickness  ]; %% The variables had to be positioned differently
MRS_Rot(:,1)=rda.column.'.* [1 -1 1]'; %%Both of them had to be multiplied by the [1 -1 1]
MRS_Rot(:,2)=rda.row.'.* [1 -1 1]';
...
% Below displays the mask on the image. Turned off for now. Would be nice
% to save these in the folder
